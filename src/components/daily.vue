					...
					<span v-for="day in days" :key="item.dailyId+'-day'+day" class="hover active" 
						...
						@click="
							...
							if(done){
								...
								$axios.post('/banban/daily/undone',$qs.stringify({
								...
								var doneDate = $moment(fromDate).subtract(day,'day')
								$axios.post('/banban/daily/done',$qs.stringify({
									dailyId:item.dailyId,
									date:doneDate.format('YYYY-MM-DD')
								})).then(res=>{
									if(res.data.codeMsg)
						...
						@click="
							...
							if(r){
								if(r != chosenDaily.name){
									$axios.post('/banban/daily/update-daily',$qs.stringify({
					...
					<span class="active" 
						...
						@click="
							$dialog.confirm({message:'确认删除吗?'}).then(()=>{
								$axios.post('/banban/daily/delete-daily',$qs.stringify({dailyId:chosenDaily.dailyId})).then(res=>{
									if(res.data.codeMsg)
										$dialog.alert({message:res.data.codeMsg})
									if(res.data.code == 0){
										if(!res.data.codeMsg)
		...
		<div style="height:91px;position: absolute;bottom:0;left:0;right:0;background-color: #FFFFFF;box-shadow: 0px -1px 4px 0px #6b6b6b;">
			<div class="active" style="height:40px;text-align: center;cursor:pointer;position: relative;"
				@click="
					...
					if(r){
						$axios.post('/banban/daily/create-daily',$qs.stringify({name:r})).then(res=>{
							if(res.data.codeMsg)
								$dialog.alert({message:res.data.codeMsg})
							if(res.data.code == 0){
								if(!res.data.codeMsg)
