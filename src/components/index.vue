	...
	<div id="index" style="font-size: 0;position:absolute;top:0;bottom:0;right:0;left:0;">
		<div style="height: 30px;border-bottom:1px solid #8f8f8f;overflow: hidden;">
				...
				<input v-model="kw" type="text" 
					...
					@focus="
						...
						if($store.state.login){
							$axios.get(`/banban/cache/get?name=${'queryHistory.kws-'+$store.state.login.userId}`)
								.then(res=>{
									if(res.data.code == 0)
			...
			<span class="active"  
				@click="
					kindId=''
					sort=order=status=kw=null
					type=status=''
					faQiRenUserId=fuZeRenUserId=null
	...
	export default {
		...
		methods: {
			load() {
				...
				vue.$axios.get('/banban/my-task/my-fa-qi-rens?'+vue.$qs.stringify({
				...
				vue.$axios.get('/banban/my-task/my-fu-ze-rens?'+vue.$qs.stringify({
					kindId:vue.kindId==null?undefined:vue.kindId,
					sort:'orderNo',
					order:'asc'
				})).then(res => {
			...
			loadTasks(){
				...
				if(vue.kw && vue.queryHistory.kws.indexOf(vue.kw)==-1){
					...
					if(vue.$store.state.login){
						vue.$axios.post('/banban/cache/set',vue.$qs.stringify({
				...
				vue.$axios.get('/banban/my-task/tasks?' + vue.$qs.stringify(param)).then(res => {
				...
				vue.$axios.get('/banban/my-task/tasks-sum?' + vue.$qs.stringify(param)).then(res => {
