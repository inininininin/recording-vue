	...
	<div id="index" style="height:100%;position:relative;">
		<div class="scrollbar1"
			...
			<span style="line-height: 30px;font-size: 14px;cursor: pointer;display:inline-block;border-right:1px solid #8f8f8f;">
				<select v-model="status" :style="{color:status?'red':'#000000'}" @change="pn=1;taskList=[];queryHistoryPad=0;loadTaskList()" 
					style="height:30px;border:none;cursor: pointer;padding: 0px 5px;">
			...
			<span style="line-height: 30px;font-size: 14px;cursor: pointer;display:inline-block;border-right:1px solid #8f8f8f;">
				<select v-model="type" :style="{color:type?'red':'#000000'}" @change="pn=1;taskList=[];queryHistoryPad=0;loadTaskList()" 
			...
			<span style="line-height:20px;cursor: pointer;display:inline-block;border-right:1px solid #8f8f8f;padding: 0 5px;display:none;" >
				<label for="autoRedoTomorrowIs" style="cursor: pointer;font-size: 14px; padding: 0 5px 0 0;" :style="{color:autoRedoTomorrowIs!=1?'red':'#000000'}">
					日常
				</label>
			...
			<span
				@click="
					status=1;autoRedoTomorrowIs=1;
					cancelIs='';completeIs='';sortMap.name=0;sortMap.finalHas=0;sortMap.finalTime=0;
					sortMap.orderNo=0,sortMap.createTime=0,sortMap.updateTime=0,sortMap1={},kw=null;type='',pn=1;taskList=[];
					queryHistoryPad=0;loadTaskList();" 
			...
			<span
				style="
					...
					font-size: 14px;background-color: #ffffff;cursor: pointer;display:inline-block;
					position: absolute;right:0;top:1px;border-left:1px solid #8f8f8f;">
				···
			</span>
			<div class="scrollbar1" 
				style="
				...
				<span style="padding:0 5px;font-size: 14px;cursor: pointer;display:inline-block;border-left:1px solid #8f8f8f;position: absolute;right:0px;top:0px;">
					···
				</span>
			...
			<div class="scrollbar1" 
				...
				<span  
					@click="
						...
						taskList=[],pn=1;
						queryHistoryPad=0;loadTaskList();"  
					style="padding:0 5px;font-size: 14px;cursor: pointer;display:inline-block;border-right:1px solid #8f8f8f;background-color: #d8d5d5;">
					负责人
				</span>
				<span 
					@click="
					...
					style="padding:0 5px;font-size: 14px;cursor: pointer;display:inline-block;border-right:1px solid #8f8f8f;min-width: 20px;text-align: center;">
					我
				</span>
		...
		<div class="scrollbar" @scroll="taskListScroll($event)" ref="taskList" 
			style="overflow: auto;width:100%;position: absolute;top: 125px;bottom: 88px;">
			<div :key="item.taskId" v-for="(item, i) in taskList" class="active visited" 
				...
				<div style="position:relative;margin-top:3px;">
					<span style="font-size:12px;color:#8F8F8F;">
						{{$moment(item.createTime).format(new Date().getFullYear()==new Date(item.createTime).getFullYear()? 'MM-DD, 周d, HH:mm': 'MM-DD, 周d, HH:mm, YYYY').replace('周0','周7').replace('周','')}}
					</span>
					<span v-if="!(item.fuZeRenUserId==item.faBuRenUserId && item.faBuRenUserId==$attr($store.state.login,'userId'))" 
						style="font-size:12px;color:#8F8F8F;margin-left:15px;">
						{{item.faBuRenUserId==$attr($store.state.login,"userId")?'我':item.faBuRenUserNickname}} > {{item.fuZeRenUserId==$attr($store.state.login,"userId")?'我':item.fuZeRenUserNickname }}
					</span>
					<span v-if="item.finalTime" style="font-size:12px;margin-left:15px;" 
						:style="{color: (item.completeIs||item.cancelIs)?'#8F8F8F':item.finalTime>$store.state.now.getTime()?'#ff5500':'#ff0000'}">
						{{item.finalTime?('限: '+$moment(item.finalTime).format(new Date().getFullYear()==new Date(item.finalTime).getFullYear()? "MM-DD, 周d, HH:mm": "MM-DD, 周d, HH:mm, YYYY").replace(/周0/g,'周7').replace(/周/g,'')):''}}
					</span>
		...
		<div style="position:absolute;bottom:0;left:0;right:0;background-color: #FFFFFF;box-shadow: 0px 0 1px 0px #888888;">
			...
			<div style="height:35px;line-height: 35px;">
				<span @click="$router.push({path:'/create-task',query:{time:new Date().getTime()+''}})" 
					style="font-weight: 900;font-size:14px;width:50%;display:inline-block;text-align: center;cursor:pointer;">
					发任务 +
				</span>
				<span style="border-left:1px solid #8F8F8F;position: absolute;display:inline-block;"><span style="visibility: hidden;">1</span></span>
				<span style="font-weight: 900;font-size:14px;width:50%;display:inline-block;text-align: center;cursor:pointer;">
					去记录 +
				</span>
			</div>
			<div style="border-top:1px solid #8F8F8F;"></div>
			<div style="height:50px;line-height: 50px;position: relative;">
				<span  @click="$router.replace({path:'/index',query:{time:new Date().getTime()+''}})" 
					style="font-size:14px;width:20%;display:inline-block;text-align: center;cursor:pointer;background-color: #ff7f08;">
					任务
				</span>
				<span style="font-size:14px;width:20%;display:inline-block;text-align: center;cursor:pointer;">记录</span>
				<span @click="$router.replace({path:'/friend-list',query:{time:new Date().getTime()+''}})" 
					style="font-size:14px;width:20%;display:inline-block;text-align: center;cursor:pointer;">
					好友
				</span>
				<span style="font-size:14px;width:20%;display:inline-block;text-align: center;cursor:pointer;">消息</span>
				<span @click="$router.replace({path:'/me',query:{time:new Date().getTime()+''}})" 
					style="font-size:14px;width:20%;display:inline-block;text-align: center;cursor:pointer;">
					我
				</span>
			</div>
		...
		<div v-if="queryHistoryPad" 
			...
			<div style="background-color: #ffffff;padding:0 10px 10px 10px;min-height:100px;">
				...
				<div v-if="queryHistoryKwList && queryHistoryKwList.length>0" 
					...
					style="font-size: 14px;cursor: pointer;text-align: center;margin-top:10px;color:#8F8F8F;border:1px dashed #8f8f8f;">
					清空历史记录
				</div>
				<div v-if="!queryHistoryKwList || queryHistoryKwList.length==0" 
					style="font-size: 14px;text-align: center;margin-top:10px;color:#8F8F8F;">
					暂无历史记录
				</div>
			</div>
		</div>
